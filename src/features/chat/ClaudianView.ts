...
export class ClaudianView extends ItemView {
  ...
  private wireEventHandlers(): void {
    ...
    // Alt+K (Option+K on macOS): add a line-range @mention for the current editor
    // selection, or inline the selection as an <editor_selection> block in reading view.
    this.registerDomEvent(activeDocument, 'keydown', (e: KeyboardEvent) => {
      ...
      // Reject the sentinel notePath set when the active view is not a vault note.
      ...
      if (ctx.startLine !== undefined) {
        // Source mode: line numbers are known — insert a mention token and register
        // the file + range for send-time resolution.
        const start = ctx.startLine;
        const end = start + (ctx.lineCount ?? 1) - 1;
        const mentionText = start === end
          ? `@${filename}#${start}`
          : `@${filename}#${start}-${end}`;
        ...
        // Reading mode: no line numbers — inline the selected text directly.
    ...
    // Shift+drop a Vault file into the chat to insert an @mention. Captured on the
    // document so we intercept before Obsidian's own drop handling; without Shift the
    // drop falls through to the default image/attachment handling.
