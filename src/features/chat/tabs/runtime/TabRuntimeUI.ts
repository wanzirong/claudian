...
function buildInputToolbar(
  ...
  runtimeRef: PublishedTabRuntimeRef,
  onUserModified: () => void,
): ReturnType<typeof createInputToolbar> & { sendStopButton: SendStopButton } {
  const { dom } = shell;
  const { plugin } = options;
  ...
  const sendStopButton = new SendStopButton(inputToolbar, {
    onSend: () => {
      const tab = runtimeRef.current();
      void tab?.controllers.inputController.sendMessage();
    },
    onStop: () => {
      runtimeRef.current()?.controllers.inputController.cancelStreaming();
    },
  });
  return { ...toolbarComponents, sendStopButton };
}
